...
test("server-renders the public-benefit project home page", async () => {
  ...
  assert.match(html, /<title>MC 自动翻译工具｜1\.3\.8 正式版<\/title>/i);
  ...
  assert.match(html, /1\.3\.8 正式版/);
  assert.match(html, /15 个经过校验的 JAR/);
  assert.match(html, /自定义 API/);
  assert.match(html, /从 GitHub 下载/);
  const releaseDownloads = html.match(
    /https:\/\/github\.com\/wuxiangdan96-byte\/mc-auto-translation-tool\/releases\/download\/v1\.3\.8\/[A-Za-z0-9.-]+/g,
  ) ?? [];
  assert.equal(new Set(releaseDownloads).size, 16);
  assert.match(html, /MCAutoTranslationTool-1\.3\.8-mc1\.8\.9-forge\.jar/);
  assert.match(html, /MCAutoTranslationTool-1\.3\.8-fabric-all\.jar/);
  assert.match(html, /MCAutoTranslationTool-1\.3\.8-mc1\.21\.9-1\.21\.11-forge\.jar/);
  assert.match(html, /MCAutoTranslationTool-1\.3\.8-mc26\.1-26\.1\.2-forge\.jar/);
